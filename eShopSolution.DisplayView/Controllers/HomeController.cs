...
namespace eShopSolution.DisplayView.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public HomeController(EShopSolutionDBContext context, ICartService cartService, IProductService productService)
        {
            _context = context;
        ...
        public IActionResult  Cart()
        {
            ...
            var list = from x in _context.Products
                       from y in _context.Carts
                       from z in _context.AppUsers
                       where y.ProductId == x.Id && y.UserId == z.Id && z.Id.ToString() == userId
                       select x;
            ViewBag.list = list.ToArray();
            return View();
        }

        public async Task<IActionResult> CreateCart(CartCreateRequest request)
        {
            ...
            if (result != null)
            {
                TempData["result"] = "Thêm mới sản phẩm vào giỏ hàng  thành công";
                return RedirectToAction("Cart");
            }
            ModelState.AddModelError("", "Thêm mới sản phẩm vào giỏ hàng thất bại");
            return View(request);
        }
